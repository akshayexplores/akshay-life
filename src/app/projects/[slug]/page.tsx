...
import { getAllProjects, getProject, renderMarkdown } from "@/lib/mdx"
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      description: doc.meta.excerpt || undefined,
      type: "article",
      url: `https://akshay.life/projects/${slug}`,
    },
    twitter: {
    ...
    alternates: { canonical: `https://akshay.life/projects/${slug}` },
  }
}
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="relative z-[2] mx-auto max-w-[720px] px-6 pt-32 md:pt-40">
      <Link href="/projects" className="t-meta link-u">← All projects</Link>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-x-4 gap-y-1">
        ...
        {meta.client && (
          <>
            ...
            <span className="t-meta">{meta.client}</span>
          </>
        )}
        {meta.year && (
          <>
            <span className="t-meta">·</span>
            <span className="t-meta">{meta.year}</span>
          </>
        )}
        <span className="t-meta">·</span>
      ...
      <nav className="mt-16 grid grid-cols-1 gap-4 md:grid-cols-2" style={{ borderTop: "1px solid var(--border)", paddingTop: "2rem" }}>
        {prev ? (
          <Link href={`/projects/${prev.slug}`} className="bento block p-5">
            <p className="t-label">Newer</p>
            <p className="mt-2 font-body" style={{ color: "var(--text)", lineHeight: 1.5 }}>{prev.title}</p>
          </Link>
        ) : <span />}
        {next ? (
          <Link href={`/projects/${next.slug}`} className="bento block p-5 md:text-right">
